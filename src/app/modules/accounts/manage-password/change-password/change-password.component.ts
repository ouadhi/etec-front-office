...
export class ChangePasswordComponent extends BaseComponent implements OnInit {
  ...
  upperLettersPatern = new RegExp("^(?=.*[A-Z]).+$");
  lowerLettersPatern = new RegExp("^(?=.*[a-z]).+$");
  numbersPatern = new RegExp("^(?=.*\\d).+$");
  symbolsPatern = new RegExp("^(?=.*[-+_!@#$%^&*., ?]).+$");
  ...
  confirm() {
    ...
    this.sub = this.managePasswordService
      .resetPassword(this.userId, this.token, this.formGroup.value.password)
      .subscribe(
        (data) => {
          this.formioLoader.loading = false;
