...
export class RequestQueryComponent extends BaseComponent implements OnInit, OnDestroy {
			...
			if (this.query.controls.requestDate.value) {
				...
				this.queryData['requestDate.greaterOrEqualThan'] =
					`${this.datePipe.transform(
						this.query.controls.requestDate.value,
						'yyyy-MM-ddT00:00:00'
					)}` + 'Z';
				this.queryData['requestDate.lessThan'] =
					`${this.datePipe.transform(tomorrow, 'yyyy-MM-ddT00:00:00')}` + 'Z';
				...
				.subscribe(
					(data) => {
						if (data.length) {
							...
							this.sub = this.rest
								.getTaskByProcessInstanceId({ processInstanceId: this.data.procInstID })
								.subscribe((data) => {
									this.tasks = data;
								});
						} else {
	...
	async ngOnInit(): Promise<void> {
		...
		this.sub = this.query.controls.requestType.statusChanges
		...
		this.sub = this.query.controls.requestDate.statusChanges
			.pipe(debounceTime(200))
			.subscribe(() => {
		...
		if (this.queryData['requestDate.greaterOrEqualThan'])
			this.query.controls.requestDate.setValue(
				new Date(this.queryData['requestDate.greaterOrEqualThan'])
			);
